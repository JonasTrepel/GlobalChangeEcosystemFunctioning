...
for(year in years){
  ...
  for (i in 1:1000) {
    ...
    drive_files <- drive_ls(path = "rgee_backup", pattern = "annual_sos_doy_2") %>% 
      dplyr::select(name)
    ...
    if (n_distinct(drive_files) == 0) {
      ...
      drive_files <- drive_ls(path = "rgee_backup", pattern = "annual_sos_doy_2") %>%  dplyr::select(name)
      print(drive_files)
  ...
  files <- list.files("data/rawData/raw_time_series/phenology/phenology_tmp_tiles/",
                      full.names = T, 
                      pattern = "annual_sos_doy_2")
  
  raster_list <- lapply(files, rast)
  
  file_name_merge <- paste0("data/rawData/raw_time_series/phenology/doy_greenup_2_modis_500m_", year, ".tif")
  ...
  print(paste0(year, " SOS 2 DOY done"))
