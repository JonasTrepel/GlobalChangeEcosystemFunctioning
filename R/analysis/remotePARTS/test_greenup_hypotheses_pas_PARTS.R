...
dt_greenup <- dt %>%
  dplyr::select(all_of(greenup_cols), functional_biome, lon, lat, unique_id, lon_pa, lat_pa,
...
gls_h5.0 <- fitGLS_partition(greenup_coef ~ 1
                             partmat = pm_pa,
                             covar_FUN = "covar_exp",
                             covar.pars = list(range = range_opt_greenup),
                             data = dt_pa_sum,
...
library(gridExtra)
p_greenup_a <- grid.arrange(p_h4, p_h4.1, p_h5, p_h5.1, ncol = 4)
...
dt_est <- rbind(
  ...
  dt_est_h5 %>% mutate(model = "H5.0", t.stat = NA, response = "greenup"), 
