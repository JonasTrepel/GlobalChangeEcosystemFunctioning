...
####### calculate median_evi trend #######

median_evi_cols <- grep("median_evi_", names(dt), value = T)

#subset to complete cases
dt_median_evi <- dt %>%
  dplyr::select(all_of(median_evi_cols), functional_biome, lon, lat, unique_id, 
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.))

table(dt_median_evi$protection_cat_broad)
table(dt_median_evi$super_biome)


#get y matrix
y_median_evi <- as.matrix(dt_median_evi[, median_evi_cols])

#get coordinate matrix 
coords_median_evi <- as.matrix(dt_median_evi[, c("lon", "lat")])

#fit autoregression, accounting for temporal autocorrelation 
ar_median_evi <- fitAR_map(Y = y_median_evi, coords = coords_median_evi)

#extract coefficients and p values 
dt_median_evi$median_evi_coef <- ar_median_evi$coefficients[, "t"]
dt_median_evi$abs_median_evi_coef <- abs(ar_median_evi$coefficients[, "t"])
dt_median_evi$median_evi_p_value <- ar_median_evi$pvals[, 2]

fwrite(dt_median_evi %>% dplyr::select(
  unique_id, median_evi_coef, abs_median_evi_coef, median_evi_p_value), "data/processedData/dataFragments/pas_median_evi_trends.csv")

# get distance matrix 
#d_median_evi <- distm_scaled(coords_median_evi)

### estimate optimal r parameter (range of spatial autocorrelation)
corfit <- fitCor(resids = residuals(ar_median_evi), coords = coords_median_evi, covar_FUN = "covar_exp", 
                 start = list(range = 0.1), fit.n = nrow(dt_median_evi))
(range_opt_median_evi = corfit$spcor)

#use r to calculate optimal v parameter 
#v_opt_median_evi <- covar_exp(d_median_evi, range_opt_median_evi)

############ test hypotheses ############ 

#partition the data 
set.seed(161)
pm <- sample_partitions(npix = nrow(dt_median_evi), partsize = 1500, npart = NA)
dim(pm)

## Hypothesis 1: Ecosystem functioning is overall changing --------------------
set.seed(161)
gls_h1 <- fitGLS_partition(median_evi_coef ~ 1,
                           partmat = pm,
                           covar_FUN = "covar_exp",
                           covar.pars = list(range = range_opt_median_evi),
                           data = dt_median_evi,
                           nugget = NA,
                           ncores = 10,
                           progressbar = TRUE, 
...
set.seed(161)
gls_h2 <- fitGLS_partition(median_evi_coef ~ 1 +
                             nitrogen_depo +
                             mat_coef + 
                             max_temp_coef + 
                             map_coef + 
...
p_h2 <- dt_est_h2 %>% 
  ...
  labs(title = "H2: median_evi change ~\nglobal change", subtitle = paste0("n = ", nrow(dt_median_evi)), y = NULL, x = NULL) +
  theme_classic() +
...
set.seed(161)
gls_h3 <- fitGLS_partition(median_evi_coef ~ 0 +
                             super_biome,
                           partmat = pm,
                           covar_FUN = "covar_exp",
                           covar.pars = list(range = range_opt_median_evi),
...
p_h3 <- dt_est_h3 %>% 
  ...
  geom_pointrange(aes(y = term, x = estimate, xmin = ci_lb, xmax = ci_ub, color = sig),
                  alpha = 0.9, linewidth = 1.2) +
  scale_color_manual(values = c("significant" = "forestgreen", "non-significant" = "grey")) +
  labs(title = "H3: median_evi change ~\nsuper biome", subtitle = paste0("n = ", nrow(dt_median_evi)), y = NULL, x = NULL) +
  theme_classic() +
  theme(legend.position = "none", 
        plot.title = element_text(size = 12))
p_h3
...
set.seed(161)
gls_h4 <- fitGLS_partition(median_evi_coef ~ 0 +
                             protection_cat_broad,
                           partmat = pm,
                           covar_FUN = "covar_exp",
                           covar.pars = list(range = range_opt_median_evi),
...
p_h4 <- dt_est_h4 %>% 
  ...
  geom_pointrange(aes(y = term, x = estimate, xmin = ci_lb, xmax = ci_ub, color = sig),
                  alpha = 0.9, linewidth = 1.2) +
  scale_color_manual(values = c("significant" = "forestgreen", "non-significant" = "grey")) +
  labs(title = "H4: median_evi change ~\nprotection", subtitle = paste0("n = ", nrow(dt_median_evi)), y = NULL, x = NULL) +
  theme_classic() +
  theme(legend.position = "none", 
        plot.title = element_text(size = 12))
p_h4
...
set.seed(161)
gls_h4.1 <- fitGLS_partition(abs_median_evi_coef ~ 0 +
                               protection_cat_broad,
                             partmat = pm,
                             covar_FUN = "covar_exp",
                             covar.pars = list(range = range_opt_median_evi),
...
p_h4.1 <- dt_est_h4.1 %>% 
  ...
  geom_pointrange(aes(y = term, x = estimate, xmin = ci_lb, xmax = ci_ub, color = sig),
                  alpha = 0.9, linewidth = 1.2) +
  scale_color_manual(values = c("significant" = "forestgreen", "non-significant" = "grey")) +
  labs(title = "H4.1: abs median_evi change ~\nprotection", subtitle = paste0("n = ", nrow(dt_median_evi)), y = NULL, x = NULL) +
  theme_classic() +
  theme(legend.position = "none", 
        plot.title = element_text(size = 12))
p_h4.1
...
#get y matrix
y_pa <- as.matrix(dt_pa[, median_evi_cols])
...
#extract coefficients and p values 
dt_pa$median_evi_coef <- ar_pa$coefficients[, "t"]
dt_pa$abs_median_evi_coef <- abs(ar_pa$coefficients[, "t"])
dt_pa$median_evi_p_value <- ar_pa$pvals[, 2]
...
# change --- 
gls_h5 <- fitGLS_partition(median_evi_coef ~ 1 +
                             area_km2_log + 
                             pa_age_log,
                           partmat = pm_pa,
                           covar_FUN = "covar_exp",
...
gls_h5.1 <- fitGLS_partition(abs_median_evi_coef ~ 1 +
                               area_km2_log + 
                               pa_age_log,
                             partmat = pm_pa,
                             covar_FUN = "covar_exp",
...
p_h5.1 <- dt_est_h5.1 %>% 
  ...
  geom_pointrange(aes(y = term, x = estimate, xmin = ci_lb, xmax = ci_ub, color = sig),
                  alpha = 0.9, linewidth = 1.2) +
  scale_color_manual(values = c("significant" = "forestgreen", "non-significant" = "grey")) +
  labs(title = "H5.1: abs median_evi change ~\npa age & size", subtitle = paste0("n = ", nrow(dt_pa)), y = NULL, x = NULL) +
  theme_classic() +
  theme(legend.position = "none", 
        plot.title = element_text(size = 12)) 
p_h5.1
...
library(gridExtra)
p_median_evi_a <- grid.arrange(p_h2, p_h3, p_h4, p_h5, ncol = 4)
ggsave(plot = p_median_evi_a, "builds/plots/median_evi_remotePARTS_pas.png", dpi = 600, height = 3, width = 12)
...
biome_gls <- function(super_b = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  #extract coefficients and p values 
  dt_biome$median_evi_coef <- ar_biome$coefficients[, "t"]
  dt_biome$median_evi_p_value <- ar_biome$pvals[, 2]
  dt_biome$abs_median_evi_coef <- abs(ar_biome$coefficients[, "t"])
  ...
  if(part == FALSE){
    ...
    set.seed(161)
    gls_biome <- fitGLS(median_evi_coef ~ 1 +
                          nitrogen_depo +
                          mat_coef +
                          max_temp_coef +
                          map_coef +
  ...
  if(part == TRUE){
    ...
    set.seed(161)
    gls_biome <- fitGLS_partition(median_evi_coef ~ 1 +
                                    nitrogen_depo +
                                    mat_coef + 
                                    max_temp_coef + 
                                    map_coef + 
...
p_cold_tall <- biome_gls(super_b = "cold_tall", fit_n = "row_n", part_size = 1500,
                         col_pattern = "median_evi_", dat = dt, part = FALSE, start = list(range = 0.1))
...
p_not_cold_tall <- biome_gls(super_b = "not_cold_tall", fit_n = "row_n", part_size = 1000,
                             col_pattern = "median_evi_", dat = dt, part = TRUE, start = list(range = 0.5))
...
p_median_evi_super_b <- gridExtra::grid.arrange(p_cold_tall, p_cold_short, p_not_cold_tall, p_not_cold_short,
                                                ncol = 4)
ggsave(plot = p_median_evi_super_b, "builds/plots/median_evi_remotePARTS_super_b_pas.png", dpi = 600, height = 3, width = 12)

p_median_evi <- gridExtra::grid.arrange(p_median_evi_a, p_median_evi_super_b, ncol = 1)
ggsave(plot = p_median_evi, "builds/plots/median_evi_remotePARTS_full_pas.png", dpi = 600, height = 5, width = 14)
...
fwrite(dt_est, "builds/model_estimates/median_evi_pas_estimates.csv")
