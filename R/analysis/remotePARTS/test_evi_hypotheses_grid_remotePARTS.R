...
p_cold <- biome_gls(ndvi_m = "cold", col_pattern = "evi_", dat = dt, part = TRUE, start = list(range = 0.1), fit_n = 15000)
p_cold

nrow(dt[dt$ndvi_min == "dry",])
p_dry <- biome_gls(ndvi_m = "dry", col_pattern = "evi_", dat = dt, part = TRUE, fit_n = 15000)
p_dry

nrow(dt[dt$ndvi_min == "cold_and_dry",])
p_cold_and_dry <- biome_gls(ndvi_m = "cold_and_dry", col_pattern = "evi_", dat = dt, part = TRUE, fit_n = 15000)
p_cold_and_dry

nrow(dt[dt$ndvi_min == "non_seasonal",])
p_non_seasonal <- biome_gls(ndvi_m = "non_seasonal", col_pattern = "evi_", dat = dt, part = TRUE, fit_n = 15000)
p_non_seasonal

## 
nrow(dt[dt$productivity == "low",])
p_low <- biome_gls(prod = "low", col_pattern = "evi_", dat = dt, part = TRUE, fit_n = 15000)
p_low

nrow(dt[dt$productivity == "medium",])
p_medium <- biome_gls(prod = "medium", col_pattern = "evi_", dat = dt, part = TRUE, fit_n = 15000)
p_medium
...
ggsave(plot = p_evi_ndvi, "builds/plots/grid_evi_remotePARTS_ndvi.png", dpi = 600, height = 3, width = 14)

p_evi_prod <- gridExtra::grid.arrange(p_low, p_medium, p_high, ncol = 5)
ggsave(plot = p_evi_prod, "builds/plots/grid_evi_remotePARTS_prod.png", dpi = 600, height = 3, width = 14)

p_evi <- gridExtra::grid.arrange(p_evi_a, p_evi_ndvi, p_evi_prod, ncol = 1)
ggsave(plot = p_evi, "builds/plots/grid_evi_remotePARTS_full.png", dpi = 600, height = 8, width = 14)
