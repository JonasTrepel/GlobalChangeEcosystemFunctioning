...
dt <- dt_raw %>% 
  ...
  mutate(
    ...
    mean_evi_coef = scale(mean_evi_coef),
    burned_area_coef = scale(burned_area_coef),
    greenup_coef = scale(greenup_coef)
    ) 
...
dt_est <- rbind(
  dt_est_mean_evi %>% mutate(model = "inc_other_trends", t.stat = NA, response = "mean_evi"),
  dt_est_burned_area %>% mutate(model = "inc_other_trends", t.stat = NA, response = "burned_area"), 
  dt_est_greenup %>% mutate(model = "inc_other_trends", t.stat = NA, response = "greenup")
)

fwrite(dt_est, "builds/model_estimates/global_models_with_other_trends.csv")
# Plot --------------------------------
