...
dt_burned_area <- dt %>%
  dplyr::select(all_of(burned_area_cols), functional_biome, X, Y, unique_id,
...
fwrite(dt_burned_area %>% dplyr::select(
  unique_id, X, Y, og_layer, burned_area_coef, abs_burned_area_coef, burned_area_p_value),
  "data/processedData/dataFragments/pa_burned_area_trends.csv")
