...
dt_greenup <- dt %>%
  dplyr::select(all_of(greenup_cols), functional_biome, X, Y, unique_id, productivity, ndvi_min,
...
fwrite(dt_greenup %>% dplyr::select(
  unique_id, X, Y, protection_cat_broad, greenup_coef, abs_greenup_coef, greenup_p_value), "data/processedData/dataFragments/grid_greenup_trends.csv")
...
gls_h1 # No. Est: 0.03749003 ; SE: 0.07625193  ; pval.t: 0.6229608
...
p_h3 <- dt_est_h3 %>% 
  ...
  geom_pointrange(aes(y = term, x = estimate, xmin = ci_lb, xmax = ci_ub, color = sig),
                  alpha = 0.9, linewidth = 1.2) +
  scale_color_manual(values = c("significant" = "darkturquoise", "non-significant" = "grey")) +
  labs(title = "H3: greenup change ~\nproductivity", subtitle = paste0("n = ", nrow(dt_greenup)), y = NULL, x = NULL) +
  theme_classic() +
  theme(legend.position = "none", 
        plot.title = element_text(size = 12))
p_h3
...
p_h4 <- dt_est_h4 %>% 
  ...
  geom_pointrange(aes(y = term, x = estimate, xmin = ci_lb, xmax = ci_ub, color = sig),
                  alpha = 0.9, linewidth = 1.2) +
  scale_color_manual(values = c("significant" = "darkturquoise", "non-significant" = "grey")) +
  labs(title = "H4: greenup change ~\ngrowth lim.", subtitle = paste0("n = ", nrow(dt_greenup)), y = NULL, x = NULL) +
  theme_classic() +
  theme(legend.position = "none", 
        plot.title = element_text(size = 12))
p_h4
...
gls_h6 <- fitGLS_partition(greenup_coef ~ 1 +
                             area_km2_log + 
                             pa_age_log,
                           partmat = pm_pa,
                           covar_FUN = "covar_exp",
...
p_greenup_a <- grid.arrange(p_h2, p_h3, p_h4, p_h5, p_h6, ncol = 5)
ggsave(plot = p_greenup_a, "builds/plots/grid_greenup_remotePARTS.png", dpi = 600, height = 3, width = 13)
...
biome_gls <- function(ndvi_m = NA, prod = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  if(!is.na(prod)){
    dat <- dat %>% filter(productivity %in% prod) %>%
      ...
      dplyr::select(all_of(greenup_cols), functional_biome, X, Y, ndvi_min,
                    nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification) %>% 
      filter(complete.cases(.))
  }
  ...
  dt_biome$abs_greenup_coef <- abs(ar_biome$coefficients[, "t"])
  ...
  if(part == TRUE){
    ...
    set.seed(161)
    gls_biome <- fitGLS_partition(greenup_coef ~ 1 +
                                    nitrogen_depo +
                                    mat_coef + 
                                    max_temp_coef + 
                                    map_coef + 
...
p_cold <- biome_gls(ndvi_m = "cold", col_pattern = "greenup_", dat = dt, part = TRUE, start = list(range = 0.1), fit_n = 15000)
p_cold

nrow(dt[dt$ndvi_min == "dry",])
p_dry <- biome_gls(ndvi_m = "dry", col_pattern = "greenup_", dat = dt, part = TRUE, fit_n = 15000)
p_dry

nrow(dt[dt$ndvi_min == "cold_and_dry",])
p_cold_and_dry <- biome_gls(ndvi_m = "cold_and_dry", col_pattern = "greenup_", dat = dt, part = TRUE, fit_n = 15000)
p_cold_and_dry

# nrow(dt[dt$ndvi_min == "non_seasonal",])
# p_non_seasonal <- biome_gls(ndvi_m = "non_seasonal", col_pattern = "greenup_", dat = dt, part = TRUE, fit_n = 15000)
# p_non_seasonal

## 
nrow(dt[dt$productivity == "low",])
p_low <- biome_gls(prod = "low", col_pattern = "greenup_", dat = dt, part = TRUE, fit_n = 15000)
p_low

nrow(dt[dt$productivity == "medium",])
p_medium <- biome_gls(prod = "medium", col_pattern = "greenup_", dat = dt, part = TRUE, fit_n = 15000)
p_medium

nrow(dt[dt$productivity == "high",])
p_high <- biome_gls(prod = "high", col_pattern = "greenup_", dat = dt, part = FALSE, fit_n = 15000)
p_high


p_greenup_ndvi <- gridExtra::grid.arrange(p_cold, p_dry, 
                                      p_cold_and_dry, p_non_seasonal, ncol = 5)
ggsave(plot = p_greenup_ndvi, "builds/plots/grid_greenup_remotePARTS_ndvi.png", dpi = 600, height = 3, width = 14)

p_greenup_prod <- gridExtra::grid.arrange(p_low, p_medium, p_high, ncol = 5)
ggsave(plot = p_greenup_prod, "builds/plots/grid_greenup_remotePARTS_prod.png", dpi = 600, height = 3, width = 14)

p_greenup <- gridExtra::grid.arrange(p_greenup_a, p_greenup_ndvi, p_greenup_prod, ncol = 1)
ggsave(plot = p_greenup, "builds/plots/grid_greenup_remotePARTS_full.png", dpi = 600, height = 8, width = 14)
