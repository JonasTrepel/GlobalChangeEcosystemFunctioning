...
### evi = annual median evi 
### envi = annual mean evi 
...
####### calculate envi trend #######

envi_cols <- grep("envi_", names(dt), value = T)

#subset to complete cases
dt_envi <- dt %>%
  dplyr::select(all_of(envi_cols), functional_biome, X, Y, unique_id, productivity, ndvi_min,
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.))

#get y matrix
y_envi <- as.matrix(dt_envi[, envi_cols])

#get coordinate matrix 
coords_envi <- as.matrix(dt_envi[, c("X", "Y")])

#fit autoregression, accounting for temporal autocorrelation 
ar_envi <- fitAR_map(Y = y_envi, coords = coords_envi)

#extract coefficients and p values 
dt_envi$envi_coef <- ar_envi$coefficients[, "t"]
dt_envi$abs_envi_coef <- abs(ar_envi$coefficients[, "t"])
dt_envi$envi_p_value <- ar_envi$pvals[, 2]

fwrite(dt_envi %>% dplyr::select(
  unique_id, X, Y, protection_cat_broad, envi_coef, abs_envi_coef, envi_p_value), "data/processedData/dataFragments/grid_envi_trends.csv")

# get distance matrix 
#d_envi <- distm_scaled(coords_envi)

### estimate optimal r parameter (range of spatial autocorrelation)
corfit <- fitCor(resids = residuals(ar_envi), coords = coords_envi, covar_FUN = "covar_exp", 
                 start = list(range = 0.1), fit.n = 15000)
(range_opt_envi = corfit$spcor)

#use r to calculate optimal v parameter 
#v_opt_envi <- covar_exp(d_envi, range_opt_envi)

############ test hypotheses ############ 

#partition the data 
set.seed(161)
pm <- sample_partitions(npix = nrow(dt_envi), partsize = 1500, npart = NA)
dim(pm)

## Hypothesis 1: Ecosystem functioning is overall changing --------------------
set.seed(161)
gls_h1 <- fitGLS_partition(envi_coef ~ 1,
                           partmat = pm,
                           covar_FUN = "covar_exp",
                           covar.pars = list(range = range_opt_envi),
                           data = dt_envi,
                           nugget = NA,
                           ncores = 25,
                           progressbar = TRUE, 
...
set.seed(161)
gls_h2 <- fitGLS_partition(envi_coef ~ 1 +
                             nitrogen_depo +
                             mat_coef + 
                             max_temp_coef + 
                             map_coef + 
...
set.seed(161)
gls_h3 <- fitGLS_partition(envi_coef ~ 0 +
                             super_biome,
                           partmat = pm,
                           covar_FUN = "covar_exp",
                           covar.pars = list(range = range_opt_envi),
...
set.seed(161)
gls_h4 <- fitGLS_partition(envi_coef ~ 0 +
                             protection_cat_broad,
                           partmat = pm,
                           covar_FUN = "covar_exp",
                           covar.pars = list(range = range_opt_envi),
...
set.seed(161)
gls_h4.1 <- fitGLS_partition(abs_envi_coef ~ 0 +
                               protection_cat_broad,
                             partmat = pm,
                             covar_FUN = "covar_exp",
                             covar.pars = list(range = range_opt_envi),
...
#extract coefficients and p values 
dt_pa$envi_coef <- ar_pa$coefficients[, "t"]
dt_pa$abs_envi_coef <- abs(ar_pa$coefficients[, "t"])
dt_pa$envi_p_value <- ar_pa$pvals[, 2]
...
# change --- 
gls_h5 <- fitGLS_partition(envi_coef ~ 1 +
                             area_km2_log + 
                             pa_age_log,
                           partmat = pm_pa,
                           covar_FUN = "covar_exp",
...
gls_h5.1 <- fitGLS_partition(abs_envi_coef ~ 1 +
                               area_km2_log + 
                               pa_age_log,
                             partmat = pm_pa,
                             covar_FUN = "covar_exp",
...
library(gridExtra)
p_envi_a <- grid.arrange(p_h2, p_h3, p_h4, p_h5, ncol = 4)
ggsave(plot = p_envi_a, "builds/plots/envi_remotePARTS_grid.png", dpi = 600, height = 3, width = 12)
...
biome_gls <- function(super_b = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  #extract coefficients and p values 
  dt_biome$envi_coef <- ar_biome$coefficients[, "t"]
  dt_biome$envi_p_value <- ar_biome$pvals[, 2]
  dt_biome$abs_envi_coef <- abs(ar_biome$coefficients[, "t"])
  ...
  if(fit_n == "row_n"){
    ...
    print("start estimating range on a subset of the data")
  ...
  if(part == TRUE){
    ...
    set.seed(161)
    gls_biome <- fitGLS_partition(envi_coef ~ 1 +
                                    nitrogen_depo +
                                    mat_coef + 
                                    max_temp_coef + 
                                    map_coef + 
...
p_cold_tall <- biome_gls(super_b = "cold_tall", fit_n = 15000, part_size = 1500,
                         col_pattern = "envi_", dat = dt, part = TRUE, start = list(range = 0.1))
...
p_envi_super_b <- gridExtra::grid.arrange(p_cold_tall, p_cold_short, p_not_cold_tall, p_not_cold_short,
                                         ncol = 4)
ggsave(plot = p_envi_super_b, "builds/plots/envi_remotePARTS_super_b_grid.png", dpi = 600, height = 3, width = 12)

p_envi <- gridExtra::grid.arrange(p_envi_a, p_envi_super_b, ncol = 1)
ggsave(plot = p_envi, "builds/plots/envi_remotePARTS_full_grid.png", dpi = 600, height = 5, width = 14)
...
fwrite(dt_est, "builds/model_estimates/envi_grid_estimates.csv")
