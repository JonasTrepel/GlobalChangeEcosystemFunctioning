...
dt_envi <- dt %>%
  dplyr::select(all_of(envi_cols), functional_biome, X, Y, lon, lat, unique_id, 
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.))

table(dt_envi$protection_cat_broad)
table(dt_envi$super_biome)
...
coords_envi <- as.matrix(dt_envi[, c("lon", "lat")])
...
fwrite(dt_envi %>% dplyr::select(
  unique_id, envi_coef, abs_envi_coef, envi_p_value), "data/processedData/dataFragments/grid_envi_trends.csv")
