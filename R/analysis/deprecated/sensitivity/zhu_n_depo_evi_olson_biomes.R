...
biome_gls <- function(o_biome = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  dat <- dat %>% filter(olson_biome %in% o_biome) %>%
    dplyr::select(all_of(coi), olson_biome, X, Y, functional_biome, lon, lat, mean_n_depo_zhu,
...
fwrite(dt_plot, "builds/model_estimates/zhu_n_depo_mean_evi_olson_biomes_sep.csv")
...
p_est <- dt_plot %>%
  mutate(clean_term = factor(clean_term, levels = c(
    ...
    olson_biome = gsub("Conifer", "Conifer\n", olson_biome)) %>% 
  ggplot() +
  geom_vline(xintercept = 0, linetype = "dashed") +
...
ggsave(plot = p_est, "builds/plots/supplement/zhu_n_depo_mean_evi_in_olson_biomes.png", dpi = 600, height = 7, width = 10)
