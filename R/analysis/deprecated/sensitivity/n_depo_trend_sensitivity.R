...
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, n_depo_europe_coef,
...
#subset to complete cases
dt_burned_area <- dt %>%
  dplyr::select(all_of(burned_area_cols), functional_biome, X, Y, lon, lat, unique_id, n_depo_europe_coef,
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.)) %>% as.data.frame()
...
dt_greenup <- dt %>%
  ...
  dplyr::select(all_of(greenup_cols), functional_biome, X, Y, lon, lat, unique_id, n_depo_europe_coef,
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.)) %>% as.data.frame()
...
fwrite(dt_est, "builds/model_estimates/europe_global_change_models_with_ndep_trend.csv")
# Plot --------------------------------
...
p_est_europe <- dt_plot %>%
  ...
  labs(title = "Europe", y = NULL, x = "Trend Estimate", color = "Significance") +
  theme_minimal() +
...
p_est_europe
ggsave(plot = p_est, "builds/plots/supplement/europe_n_depo_trend_estimates.png", dpi = 600, height = 3, width = 9)
...
#subset to complete cases
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, n_depo_usa_coef,
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
...
#subset to complete cases
dt_burned_area <- dt %>%
  dplyr::select(all_of(burned_area_cols), functional_biome, X, Y, lon, lat, unique_id, n_depo_usa_coef,
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.)) %>% as.data.frame()
...
dt_greenup <- dt %>%
  filter(!grepl("N", functional_biome)) %>% 
  dplyr::select(all_of(greenup_cols), functional_biome, X, Y, lon, lat, unique_id, n_depo_usa_coef,
                nitrogen_depo, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
                protection_cat_broad) %>% 
  filter(complete.cases(.)) %>% as.data.frame()
...
fwrite(dt_est, "builds/model_estimates/usa_global_change_models_with_ndep_trend.csv")
# Plot --------------------------------
...
p_est_usa <- dt_plot %>%
  ...
  labs(title = "USA", y = NULL, x = "Trend Estimate", color = "Significance") +
  theme_minimal() +
  theme(legend.position = "bottom", 
        # panel.grid = element_blank(),
        strip.text = element_text(face = "bold", size = 11.5),   
...
p_est_usa
ggsave(plot = p_est, "builds/plots/supplement/usa_n_depo_trend_estimates.png", dpi = 600, height = 3, width = 9)

## combine: 

p_comb <- gridExtra::grid.arrange(p_est_usa, p_est_europe)
ggsave(plot = p_comb, "builds/plots/supplement/comb_n_depo_trend_estimates.png", dpi = 600, height = 6, width = 9)
