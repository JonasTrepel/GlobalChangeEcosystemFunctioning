...
biome_gls <- function(f_biome = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  dat <- dat %>% filter(functional_biome %in% f_biome) %>%
    dplyr::select(all_of(coi), olson_biome, X, Y, functional_biome, lon, lat,  n_depo_europe_coef,
...
fwrite(dt_plot, "builds/model_estimates/europe_mean_evi_functional_biomes_sep_n_depo_trend.csv")
dt_plot <- fread("builds/model_estimates/europe_mean_evi_functional_biomes_sep_n_depo_trend.csv")
...
ggsave(plot = p_est_functional_biome, "builds/plots/supplement/europe_mean_evi_in_functional_biomes_n_depo_trend.png", dpi = 600, height = 10, width = 10)
...
biome_gls <- function(f_biome = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  dat <- dat %>% filter(super_biome %in% f_biome) %>%
    dplyr::select(all_of(coi), olson_biome, X, Y, super_biome, lon, lat, n_depo_europe_coef,
                   mat_coef, map_coef, max_temp_coef, human_modification) %>% 
    filter(complete.cases(.))
...
fwrite(dt_plot, "builds/model_estimates/europe_mean_evi_super_biomes_sep_n_depo_trend.csv")
dt_plot <- fread("builds/model_estimates/europe_mean_evi_super_biomes_sep_n_depo_trend.csv")
...
ggsave(plot = p_est_super_biome, "builds/plots/supplement/europe_mean_evi_in_super_biomes_n_depo_trend.png", dpi = 600, height = 10, width = 10)
...
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, 
                n_depo_europe_coef, mean_n_depo_zhu, n_depo_zhu_coef,
...
ggsave(plot = p_est_global, "builds/plots/supplement/europe_mean_evi_n_depo_n_depo_trend.png", dpi = 600)

p_comb <- gridExtra::grid.arrange(p_est_functional_biome, p_est_super_biome, p_est_global)
ggsave(plot = p_comb, "builds/plots/supplement/europe_mean_evi_n_depo_estimates_n_depo_trend.png", dpi = 600, height = 6, width = 7)
