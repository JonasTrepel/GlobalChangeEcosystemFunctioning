...
fwrite(dt_plot, "builds/model_estimates/europe_mean_evi_functional_biomes_sep.csv")
dt_plot <- fread("builds/model_estimates/europe_mean_evi_functional_biomes_sep.csv")
...
p_est_functional_biome <- dt_plot %>%
  ...
  geom_pointrange(aes(x = estimate, xmin = ci_lb, xmax = ci_ub, y = clean_term, color = sig_pn),
                  alpha = 0.9, linewidth = 1.2) +
  facet_wrap(~functional_biome, scales = "free_x", ncol = 3) +
...
ggsave(plot = p_est_functional_biome, "builds/plots/supplement/europe_mean_evi_in_functional_biomes.png", dpi = 600, height = 10, width = 10)
...
fwrite(dt_plot, "builds/model_estimates/europe_mean_evi_super_biomes_sep.csv")
dt_plot <- fread("builds/model_estimates/europe_mean_evi_super_biomes_sep.csv")
...
p_est_super_biome <- dt_plot %>%
  ...
  geom_pointrange(aes(x = estimate, xmin = ci_lb, xmax = ci_ub, y = clean_term, color = sig_pn),
                  alpha = 0.9, linewidth = 1.2) +
  facet_wrap(~super_biome, scales = "free_x", ncol = 3) +
  scale_color_manual(values = c("Sig. Negative" = "#9E3C85",
                                "Non-Significant" = "grey60", 
                                "Sig. Positive" = "#457B2A"
  )) +
...
ggsave(plot = p_est_super_biome, "builds/plots/supplement/europe_mean_evi_in_super_biomes.png", dpi = 600, height = 10, width = 10)
...
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, 
                mean_n_depo_europe, n_depo_europe_coef, mean_n_depo_zhu, n_depo_zhu_coef,
...
p_est_global <- dt_plot %>%
  ...
  labs(title = "Europe", y = NULL, x = "EVI Trend Estimate", color = "Significance") +
  theme_minimal() +
  theme(legend.position = "none", 
        # panel.grid = element_blank(),
        strip.text = element_text(face = "bold", size = 11.5),   
...
ggsave(plot = p_est_global, "builds/plots/supplement/europe_mean_evi_n_depo.png", dpi = 600)


p_comb <- gridExtra::grid.arrange(p_est_functional_biome, p_est_super_biome, p_est_global)
ggsave(plot = p_comb, "builds/plots/supplement/europe_mean_evi_n_depo_estimates.png", dpi = 600, height = 6, width = 7)
...
dt_corr <- dt_mean_evi %>%
  dplyr::select(mean_n_depo_europe, n_depo_europe_coef, nitrogen_depo, mean_evi_coef,
                mean_n_depo_zhu, n_depo_zhu_coef) %>%
  filter(complete.cases(.))

cor.test(dt_corr$mean_n_depo_zhu, dt_corr$mean_n_depo_europe)
cor.test(dt_corr$mean_n_depo_europe, dt_corr$nitrogen_depo)
cor.test(dt_corr$n_depo_europe_coef, dt_corr$nitrogen_depo)
cor.test(dt_corr$mean_evi_coef, dt_corr$nitrogen_depo)
cor.test(dt_corr$mean_evi_coef, dt_corr$n_depo_europe_coef)
...
p_r1 <- dt_mean_evi %>%
  ggplot() + 
  geom_point(aes(x = nitrogen_depo, y = n_depo_europe_coef), alpha = 0.5, size = 0.7) +
  labs(x = "N depo mean", y = "Europe N depo trend") +
  theme_classic()
p_r1

p_r2 <- dt_mean_evi %>%
  ggplot() + 
  geom_point(aes(x = nitrogen_depo, y = mean_evi_coef), alpha = 0.5, size = 0.7) +
  labs(x = "N depo mean", y = "EVI trend") +
  theme_classic()
p_r2

p_r3 <- dt_mean_evi %>%
  ggplot() + 
  geom_point(aes(x = n_depo_europe_coef, y = mean_evi_coef), alpha = 0.5, size = 0.7) +
  labs(x = "Europe N depo trend", y = "EVI trend") +
  theme_classic()
p_r3


p_europe <- gridExtra::grid.arrange(p_mean, p_n_trend, p_mean_evi_trend, p_r1, p_r2, p_r3, ncol = 3)
ggsave(plot = p_europe, "builds/plots/supplement/europe_n_depo.png", dpi = 600)
