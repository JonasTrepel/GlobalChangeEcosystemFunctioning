...
dt_raw_2 <- dt_raw %>% 
  mutate(trend_cat = case_when(
    n_depo_usa_coef <= as.numeric(quantile(dt_raw$n_depo_usa_coef, c(0.33), na.rm = T)) ~ "trend_decrease", 
    n_depo_usa_coef >= as.numeric(quantile(dt_raw$n_depo_usa_coef, c(0.66), na.rm = T)) ~ "trend_increase", 
    n_depo_usa_coef > as.numeric(quantile(dt_raw$n_depo_usa_coef, c(0.33), na.rm = T)) &
      n_depo_usa_coef < as.numeric(quantile(dt_raw$n_depo_usa_coef, c(0.66), na.rm = T)) ~ "trend_stable"),
    depo_cat = case_when(
      mean_n_depo_usa <= as.numeric(quantile(dt_raw$mean_n_depo_usa, c(0.33), na.rm = T)) ~ "mean_low", 
      mean_n_depo_usa >= as.numeric(quantile(dt_raw$mean_n_depo_usa, c(0.66), na.rm = T)) ~ "mean_high", 
      mean_n_depo_usa > as.numeric(quantile(dt_raw$mean_n_depo_usa, c(0.33), na.rm = T)) &
        mean_n_depo_usa < as.numeric(quantile(dt_raw$mean_n_depo_usa, c(0.66), na.rm = T)) ~ "mean_medium"),
...
biome_gls <- function(f_biome = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  dat <- dat %>% #filter(functional_biome %in% f_biome) %>%
    dplyr::select(all_of(coi), olson_biome, X, Y, functional_biome, lon, lat,  n_depo_usa_coef, mean_n_depo_usa,
...
fwrite(dt_plot, "builds/model_estimates/usa_mean_evi_functional_biomes_sep_n_depo_syndrome.csv")
dt_plot <- fread("builds/model_estimates/usa_mean_evi_functional_biomes_sep_n_depo_syndrome.csv")
...
ggsave(plot = p_est_functional_biome, "builds/plots/supplement/usa_mean_evi_in_functional_biomes_n_depo_syndrome.png", dpi = 600, height = 10, width = 10)
...
biome_gls <- function(f_biome = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  dat <- dat %>% filter(super_biome %in% f_biome) %>%
    dplyr::select(all_of(coi), olson_biome, X, Y, super_biome, lon, lat, n_depo_usa_coef, mean_n_depo_usa,
                  nitrogen_depo, n_depo_syndrome, mat_coef, map_coef, max_temp_coef, human_modification) %>% 
    filter(complete.cases(.)) %>% 
    group_by(n_depo_syndrome) %>% 
    mutate(n_fb = n()) %>% 
    ungroup() %>% 
    filter(n_fb > 500)
...
fwrite(dt_plot, "builds/model_estimates/usa_mean_evi_super_biomes_sep_n_depo_syndrome.csv")
dt_plot <- fread("builds/model_estimates/usa_mean_evi_super_biomes_sep_n_depo_syndrome.csv")
...
ggsave(plot = p_est_super_biome, "builds/plots/supplement/usa_mean_evi_in_super_biomes_n_depo_syndrome.png", dpi = 600, height = 10, width = 10)
...
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, 
                mean_n_depo_usa, n_depo_usa_coef, mean_n_depo_zhu, n_depo_zhu_coef, n_depo_syndrome,
...
p_est_global <- dt_plot %>%
  ...
  labs(title = "USA", y = "N deposition syndrome", x = "EVI Trend Estimate", color = "Significance") +
  theme_minimal() +
  theme(legend.position = "none", 
        # panel.grid = element_blank(),
        strip.text = element_text(face = "bold", size = 11.5),   
...
ggsave(plot = p_est_global, "builds/plots/supplement/usa_mean_evi_n_depo_n_depo_syndrome.png", dpi = 600)

p_comb <- gridExtra::grid.arrange(p_est_functional_biome, p_est_super_biome, p_est_global)
ggsave(plot = p_comb, "builds/plots/supplement/usa_mean_evi_n_depo_estimates_n_depo_syndrome.png", dpi = 600, height = 6, width = 7)
...
p_est_global_exp <- dt_plot_exp %>%
  ...
  labs(title = "USA", y = "Expectation", x = "EVI Trend Estimate", color = "Significance") +
  theme_minimal() +
  theme(legend.position = "none", 
        # panel.grid = element_blank(),
        strip.text = element_text(face = "bold", size = 11.5),   
...
p_mean <- grid_usa %>%
  ...
  filter(nitrogen_depo > as.numeric(quantile(grid_usa$nitrogen_depo, 0.025, na.rm = T)) &
           nitrogen_depo < as.numeric(quantile(grid_usa$nitrogen_depo, 0.975, na.rm = T))
  ) %>% 
  ggplot() +
...
p_mean_evi_trend <- grid_usa %>%
  st_transform(crs = 4326) %>% 
  filter(mean_evi_coef > as.numeric(quantile(grid_usa$mean_evi_coef, 0.025, na.rm = T)) &
           mean_evi_coef < as.numeric(quantile(grid_usa$mean_evi_coef, 0.975, na.rm = T))
  ) %>% 
  ggplot() +
...
p_usa_1 <- gridExtra::grid.arrange(p_mean, p_n_trend, p_mean_evi_trend, ncol = 3)
...
p_usa <- gridExtra::grid.arrange(p_usa_1, p_n_depo_syndrome, p_est, ncol = 1, heights = c(1, 2, 2))


ggsave(plot = p_usa, "builds/plots/supplement/usa_n_depo.png", dpi = 600, height = 12, width = 10)
