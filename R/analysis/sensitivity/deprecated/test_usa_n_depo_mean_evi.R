...
fwrite(dt_plot, "builds/model_estimates/usa_mean_evi_functional_biomes_sep.csv")
dt_plot <- fread("builds/model_estimates/usa_mean_evi_functional_biomes_sep.csv")
...
ggsave(plot = p_est_functional_biome, "builds/plots/supplement/usa_mean_evi_in_functional_biomes.png", dpi = 600, height = 10, width = 10)
...
fwrite(dt_plot, "builds/model_estimates/usa_mean_evi_super_biomes_sep.csv")
dt_plot <- fread("builds/model_estimates/usa_mean_evi_super_biomes_sep.csv")
...
ggsave(plot = p_est_super_biome, "builds/plots/supplement/usa_mean_evi_in_super_biomes.png", dpi = 600, height = 10, width = 10)
...
ggsave(plot = p_est_global, "builds/plots/supplement/usa_mean_evi_n_depo.png", dpi = 600)

p_comb <- gridExtra::grid.arrange(p_est_functional_biome, p_est_super_biome, p_est_global)
ggsave(plot = p_comb, "builds/plots/supplement/usa_mean_evi_n_depo_estimates.png", dpi = 600, height = 6, width = 7)
...
dt_corr <- dt_mean_evi %>%
  dplyr::select(mean_n_depo_usa, n_depo_usa_coef, nitrogen_depo, mean_evi_coef,
...
cor.test(dt_corr$n_depo_usa_coef, dt_corr$nitrogen_depo)
cor.test(dt_corr$mean_evi_coef, dt_corr$nitrogen_depo)
cor.test(dt_corr$mean_evi_coef, dt_corr$n_depo_usa_coef)
...
p_r1 <- dt_mean_evi %>%
  ggplot() + 
  geom_point(aes(x = nitrogen_depo, y = n_depo_usa_coef), alpha = 0.5, size = 0.7) +
  labs(x = "N depo mean", y = "usa N depo trend") +
  theme_classic()
p_r1
...
p_r3 <- dt_mean_evi %>%
  ggplot() + 
  geom_point(aes(x = n_depo_usa_coef, y = mean_evi_coef), alpha = 0.5, size = 0.7) +
  labs(x = "usa N depo trend", y = "EVI trend") +
  theme_classic()
p_r3


p_usa <- gridExtra::grid.arrange(p_mean, p_n_trend, p_mean_evi_trend, p_r1, p_r2, p_r3, ncol = 3)
ggsave(plot = p_usa, "builds/plots/supplement/usa_n_depo.png", dpi = 600)
