...
biome_gls <- function(f_biome = NA, col_pattern = NA, start = list(range = 0.1),
  ...
  dat <- dat %>% filter(functional_biome %in% f_biome) %>%
    dplyr::select(all_of(coi), olson_biome, X, Y, functional_biome, lon, lat,  n_depo_europe_coef, mean_n_depo_europe,
                  mean_n_depo_europe, mat_coef, map_coef, max_temp_coef, human_modification) %>% 
...
dt_plot <- dt_est %>% 
  ...
  mutate(clean_term = case_when(
    ...
    grepl("mean_n_depo_europe", term) ~ "N Deposition Mean",
...
fwrite(dt_plot, "builds/model_estimates/europe_mean_evi_functional_biomes_sep_europe_n_depo_mean.csv")
dt_plot <- fread("builds/model_estimates/europe_mean_evi_functional_biomes_sep_europe_n_depo_mean.csv")
...
ggsave(plot = p_est_functional_biome, "builds/plots/supplement/europe_mean_evi_in_functional_biomes_europe_n_depo_mean.png", dpi = 600, height = 10, width = 10)
...
fwrite(dt_plot, "builds/model_estimates/europe_mean_evi_super_biomes_sep_europe_n_depo_mean.csv")
dt_plot <- fread("builds/model_estimates/europe_mean_evi_super_biomes_sep_europe_n_depo_mean.csv")
...
ggsave(plot = p_est_super_biome, "builds/plots/supplement/europe_mean_evi_in_super_biomes_europe_n_depo_mean.png", dpi = 600, height = 10, width = 10)
...
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, 
                ...
                mean_n_depo_europe, mat_coef, map_coef, max_temp_coef, human_modification, super_biome,
...
ggsave(plot = p_est_global, "builds/plots/supplement/europe_mean_evi_n_depo_mean.png", dpi = 600)

p_comb <- gridExtra::grid.arrange(p_est_functional_biome, p_est_super_biome, p_est_global)
ggsave(plot = p_comb, "builds/plots/supplement/europe_mean_evi_n_depo_estimates_europe_n_depo_mean.png", dpi = 600, height = 6, width = 7)


######MAKE THIS EUROPEAN MEAN ONLY ##########
