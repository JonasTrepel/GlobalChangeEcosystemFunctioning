...
dt_corr <- dt_raw %>%
  dplyr::select(mean_n_depo_usa, n_depo_usa_coef, nitrogen_depo, mean_n_depo_zhu) %>%
...
p_r3 <- dt_raw %>% ggplot() + 
  geom_point(aes(x = mean_n_depo_usa, y = nitrogen_depo), alpha = 0.5, size = 0.7) +
  labs(x = "USA N depo mean", y = "Rubin et al N depo") +
  theme_classic()
p_r3

p_r4 <- dt_raw %>% ggplot() + 
  geom_point(aes(x = nitrogen_depo, y = mean_n_depo_zhu), alpha = 0.5, size = 0.7) +
  labs(x = "Rubin et al N depo", y = "Zhu et al N depo") +
  theme_classic()
p_r4

p1 <- gridExtra::grid.arrange(p_mean, p_trend, ncol = 2)
p2 <- gridExtra::grid.arrange(p_r1, p_r2, p_r3, p_r4, ncol = 4)

p_c <- gridExtra::grid.arrange(p1, p2, ncol = 1)
ggsave(plot = p_c, "builds/plots/supplement/usa_n_depo_maps_and_xy.png", dpi = 600, height = 6, width = 10)
...
dt_mean_evi <- dt %>%
  dplyr::select(all_of(mean_evi_cols), functional_biome, X, Y, lon, lat, unique_id, 
                mean_n_depo_usa, n_depo_usa_coef,
...
D <- distm_scaled(coords_mean_evi)
V_opt <- covar_exp(D, range_opt_mean_evi)
...
#partition the data 
set.seed(161)
pm_mean_evi <- sample_partitions(npix = nrow(dt_mean_evi), partsize = 2000, npart = NA)
dim(pm_mean_evi) 
