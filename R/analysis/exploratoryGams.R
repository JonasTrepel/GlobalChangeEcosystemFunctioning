...
combinations <- generateCombinations(vars, max.n = 8)
...
guide <- rbind(subGuideFullRandom, subGuideFullNonRandom, subGuideBest) 
...
foreach.results <- foreach(modelGroup = unique(guide$modelGroup),
                           .packages = c('mgcv', 'ggplot2', 'tidyr', 'data.table', 'tidyverse', 'MuMIn', 'broom'
                                         ),
                           .options.snow = opts,
                           .inorder = FALSE,
                           .combine = rbindLists) %dopar% {
  ...
  resList <- list(bm.spec = bm.spec, res = res, pred = pred, pred.int = pred.int)
  return(resList)
...
toc()
stopCluster(clust)
saveRDS(foreach.results, "builds/modelOutputs/exploratoryGamRes.Rds")
