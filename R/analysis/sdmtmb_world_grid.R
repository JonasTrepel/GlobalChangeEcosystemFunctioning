...
all_mesh_results <- future_map(
  ...
  function(j) {
    ...
    formula <- as.formula(
      paste0(resp, " ~ nitrogen_depo_scaled +
                     mat_coef_scaled +
                     prec_coef_scaled +
                     hmi_change_scaled +
    ...
    cv_model_id <- paste0("cv_", tier_, "_", mesh_id, "_grid")
...
fwrite(dt_mesh_res_fin, "builds/model_outputs/cv_mesh_selection_sdmtmb_grid.csv")
...
p_loglik
ggsave(plot = p_loglik, "builds/plots/supplement/sum_loglik_different_meshs_grid_main_data.png", dpi = 600, height = 4, width = 8)


##### run models --------------------------------------------

dt_mesh_res_fin <- fread("builds/model_outputs/cv_mesh_selection_sdmtmb_grid.csv")
...
best_mesh_res_list <- future_map(1:nrow(dt_best_mesh),
                                 ...
                                 function(i) {
                                   ...
                                   n_dep_formula <- as.formula(paste0(resp, "~ nitrogen_depo_scaled"))
