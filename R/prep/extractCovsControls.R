...
pas <- read_sf("data/spatialData/protectedAreas/controlsForPas.gpkg") %>% 
  mutate(unique_id = paste0("ControlFor", controlFor)) %>% 
  rename(ContinentFromPas = continent, 
         BiomeFromPas = Biome)
...
fwrite(pasCovsDT, "data/processedData/cleanData/paControlsWithCovs.csv")
